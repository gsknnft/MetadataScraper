...
export default class ReadData {
  ...
  async readCollectedAddressData(): Promise<AddressMetadata> {
    ...
    try {
      ...
      const addressTokenIds: AddressMetadata = JSON.parse(addressTokenIdsJSON);
      return addressTokenIds;
    } catch (error) {
      throw new Error(`Error reading addressTokenIds from '${contract}_collected_metadata.json':, ${error}`);
    }
  }

  async readCollectedTokenData(): Promise<TokenMetadata> {
    const contract = process.env.CONTRACT || '';
    try {
      const addressTokenIdsPath = path.join(__dirname,`${contract}_collected_metadata.json`);
      const addressTokenIdsJSON = await fs.promises.readFile(addressTokenIdsPath, 'utf-8');
      const addressTokenIds: TokenMetadata = JSON.parse(addressTokenIdsJSON);
      return addressTokenIds;
    } catch (error) {
      throw new Error(`Error reading addressTokenIds from '${contract}_collected_metadata.json':, ${error}`);
    }
